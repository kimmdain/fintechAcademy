...
// 메뉴 입력하는 API
router.post('/franMenuInsert', function(req, res){
    ...
    var sql = "INSERT INTO fintech.franchise (name, franId, franPw, menu, price) VALUES (?, ?, ?, ?, ?)";
    connection.query(sql, [name, ID, PW, menu, price], function (error, results, fields) {
  ...
  // 메뉴 전체 조회 API
  router.post('/franMenuSelect', function(req, res){
  ...
  router.post('/franMenuUpdate', function(req, res){
    ...
    // 쿼리 수정하기
    var sql = "UPDATE fintech.franchise SET menu = ?, price = ?  WHERE franchiseCode = ?)";
    connection.query(sql, [userID], function (error, results, fields) {
        if (error) throw error;
        console.log(results[0].franPw, userPassword);
